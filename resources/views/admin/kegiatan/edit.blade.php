...
<section class="content-header">
	...
	<div class="box">
		...
		<form action="{{ url('/'.Request::segment(1).'/edit/'.$kegiatan->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
		{{ csrf_field() }}
			...
			<div class="box-body">
				<div class="col-lg-12">
					...
					<div class="form-group @if ($errors->has('nama')) has-error @endif">
						<label class="col-sm-2 control-label">{{ __('Kegiatan') }} <span class="required" style="color: #dd4b39;">*</span></label>
						<div class="col-sm-10">
							@if ($errors->has('nama'))<label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i> {{ $errors->first('nama') }}</label>@endif
							<input type="text" class="form-control" placeholder="Kegiatan" name="nama" value="{{ $kegiatan->nama }}" >
						</div>
					</div>
